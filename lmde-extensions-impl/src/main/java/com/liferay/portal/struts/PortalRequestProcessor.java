...
public class PortalRequestProcessor extends TilesRequestProcessor {
    ...
    protected boolean callParentProcessRoles(HttpServletRequest request, HttpServletResponse response, ActionMapping actionMapping) throws IOException,
            ServletException {
    ...
    protected void defineObjects(HttpServletRequest request, HttpServletResponse response, Portlet portlet) throws Exception {
        ...
        RenderRequestImpl renderRequestImpl = RenderRequestFactory.create(request, portlet, invokerPortlet, portletContext, WindowState.MAXIMIZED,
    ...
    protected String getLastPath(HttpServletRequest request) {
        ...
        if (PropsValues.COMPANY_SECURITY_AUTH_REQUIRES_HTTPS && !PropsValues.SESSION_ENABLE_PHISHING_PROTECTION && (httpsInitial != null)
    ...
    protected boolean isPortletPath(String path) {
        if ((path != null) && !path.equals(_PATH_C) && !path.startsWith(_PATH_COMMON) && !path.contains(_PATH_J_SECURITY_CHECK)
    ...
    protected boolean isPublicPath(String path) {
        if ((path != null) && (_publicPaths.contains(path) || path.startsWith(_PATH_COMMON) || AuthPublicPathRegistry.contains(path))) {
    ...
    protected Action processActionCreate(HttpServletRequest request, HttpServletResponse response, ActionMapping actionMapping) throws IOException {
        ...
        if (actionAdapter != null) {
            ActionConfig actionConfig = moduleConfig.findActionConfig(actionMapping.getPath());
    ...
    protected String processPath(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if ((userTracker != null) && !path.equals(_PATH_C) && !path.contains(_PATH_J_SECURITY_CHECK) && !path.contains(_PATH_PORTAL_PROTECTED)
        ...
        if (_lastPaths.contains(path) && !_trackerIgnorePaths.contains(path)) {
            ...
            if (themeDisplay.isLifecycleResource() || themeDisplay.isStateExclusive() || themeDisplay.isStatePopUp()
                    || !StringUtil.equalsIgnoreCase(request.getMethod(), HttpMethods.GET)) {
        ...
        if (!path.equals(_PATH_PORTAL_JSON_SERVICE) && !path.equals(_PATH_PORTAL_RENDER_PORTLET) && !ParamUtil.getBoolean(request, "wsrp")
                && !themeDisplay.isImpersonated()
                && !InterruptedPortletRequestWhitelistUtil.isPortletInvocationWhitelisted(companyId, portletId, PortalUtil.getStrutsAction(request))) {
            ...
            // CUSTOM begin : Laisse passer la page "changer mot de passe", pour éviter la boucle de redirection
            boolean pass = false;
            try {
                if (user != null) {
                    ...
                    if (Validator.isNotNull(ticketKey)) {
                        /*
                         * Pour le cas :
                         * 1. mdp perdu (envoyer un mail de reinit)
                         * 2. se connecter
                         * 3. cliquer le lien dans le mail
                         * 4. ==> supprimer le ticket reinit, rediriger vers la default landing page
                         */
                        Ticket ticket = null;
                        try {
                        ...
                        if (ticket != null && ticket.getType() == TicketConstants.TYPE_PASSWORD) {
                            ...
                            response.sendRedirect(PropsValues.DEFAULT_LANDING_PAGE_PATH);
                            return null;
                        }
                    }
                    ...
                    Group lmdeGroup = GroupLocalServiceUtil.getGroup(companyId, LmdePortalProperties.LMDE_GROUP_NAME);

                    Layout layout = LayoutLocalServiceUtil.getFriendlyURLLayout(lmdeGroup.getGroupId(), false,
                            LmdePortalProperties.LMDE_PASSWORD_CHANGE_REQUIRED_SKIP_PAGE);
            ...
            if ((user != null) && user.isPasswordReset() && !pass) {
            ...
            // CUSTOM begin : Si l'utilisateur est authentifié et qu'il accède à la page
            // /web/guest/espace-personnel/authentification alors il doit être redirigé vers la page
            // /group/guest/espace-personnel.
            try {
                if (user != null) {
                    String authType = (String) session.getAttribute("authType");
                    long plid = ParamUtil.getLong(request, "p_l_id");
                    String pstate = ParamUtil.getString(request, "p_p_state");

                    Group lmdeGroup = GroupLocalServiceUtil.getGroup(companyId, LmdePortalProperties.LMDE_GROUP_NAME);

                    Layout layout = LayoutLocalServiceUtil.getFriendlyURLLayout(lmdeGroup.getGroupId(), false,
                            LmdePortalProperties.LMDE_PAGE_AUTHENTIFICATION_FRIENDLY_URL);
                    // On ne redirige pas si c'est la pop-up facebook d'affichée
                    if (authType == null || !authType.equals(CompanyConstants.AUTH_TYPE_ID)
                            || (authType != null && authType.equals(CompanyConstants.AUTH_TYPE_ID) && pstate != null && !pstate.equals("pop_up"))) {
                        if (plid == layout.getPlid()) {
                            response.sendRedirect(PropsValues.DEFAULT_LANDING_PAGE_PATH);
                            return null;
                        }
    ...
    @Override
    protected void processPopulate(HttpServletRequest request, HttpServletResponse response, ActionForm actionForm, ActionMapping actionMapping)
