...
/**
 * Extension de la classe LoginAction permettant de gérer les différentes actions struts du Login
 * 
...
public class LoginAction extends Action {
    ...
    public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        // Début CUSTOM
        // Si c'est une authentification facebook alors on affiche un message d'erreur si pas d'utilisateur
        String connectionFrom = (String) session.getAttribute("connectionFrom");
        if (connectionFrom != null && connectionFrom.equals("facebook")) {
            login = String.valueOf(ParamUtil.getLong(request, WebKeys.USER_ID));
            if (login.equals("0")) {
                session.setAttribute("facebookFailed", "f");
            }
            session.removeAttribute("connectionFrom");
        }
        // Fin CUSTOM
