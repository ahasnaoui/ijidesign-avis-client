...
public class PortletImporter {
	...
	protected void doImportPortletInfo(
		...
		// CUSTOM LMDE -- debut
        String[] importAllCategoriesParams = portletDataContext.getParameterMap().get("importAllCategories");
        if (importAllCategoriesParams != null && GetterUtil.getBoolean(importAllCategoriesParams[0])) {
            readAllCategories(portletDataContext);
        }
        // CUSTOM LMDE -- fin
    ...
    // CUSTOM LMDE -- debut
    /**
     * Importe toutes les catégories du dossier /categories et tous les vocabulaires du dossier /vocabularies.
     * 
     * @param portletDataContext contexte de l'import
     * @throws Exception
     */
    protected void readAllCategories(PortletDataContext portletDataContext) throws Exception {

        String vocabulariesFolderPath = ExportImportPathUtil.getSourceRootPath(portletDataContext) + "/vocabularies";
        List<String> vocabularyFiles = portletDataContext.getZipFolderEntries(vocabulariesFolderPath);
        Map<Long, Long> assetVocabularyPKs = new HashMap<Long, Long>();

        try {
            for (String vocabularyFile : vocabularyFiles) {
                
                // Vocabulaires
                AssetVocabulary assetVocabulary = (AssetVocabulary)portletDataContext.getZipEntryAsObject(vocabularyFile);
                if (assetVocabulary == null) {
                    _log.warn("Durant l'import le vocabulaire suivant n'a pas pu être importé : " + vocabularyFile);
                    continue;
                }
                importAssetVocabulary(portletDataContext, assetVocabularyPKs, null, assetVocabulary);
            }
        } catch (Exception e) {
            throw new LARFileException(e);
        }

        String categoriesFolderPath = ExportImportPathUtil.getSourceRootPath(portletDataContext) + "/categories";
        List<String> categoryFiles = portletDataContext.getZipFolderEntries(categoriesFolderPath);
        Map<Long, List<AssetCategory>> childCategoryMap = new HashMap<Long, List<AssetCategory>>();
        Map<Long, Long> assetCategoryPKs = new HashMap<Long, Long>();
        Map<String, String> assetCategoryUuids = new HashMap<String, String>();
        Map<Long, Element> categoryElements = new HashMap<Long, Element>();
        try {

            for (String categoryFile : categoryFiles) {

                // Category
                AssetCategory assetCategory = (AssetCategory)portletDataContext.getZipEntryAsObject(categoryFile);
                if (assetCategory == null) {
                    _log.warn("Durant l'import la catégorie suivante n'a pas pu être importée : " + categoryFile);
                    continue;
                }
                List<AssetCategory> childCategories = childCategoryMap.get(assetCategory.getParentCategoryId());
                if (childCategories == null) {
                    childCategories = new ArrayList<AssetCategory>();
                }
                childCategories.add(assetCategory);
                childCategoryMap.put(assetCategory.getParentCategoryId(), childCategories);

                // Properties
                String xml = portletDataContext.getZipEntryAsString(categoriesFolderPath + "/properties/" + assetCategory.getCategoryId() + "-properties.xml");
                if (xml != null) {
                    Document document = SAXReaderUtil.read(xml);
                    categoryElements.put(assetCategory.getCategoryId(), document.getRootElement());
                }
            }

            addAssetCategoriesRecursive(childCategoryMap, AssetCategoryConstants.DEFAULT_PARENT_CATEGORY_ID, categoryElements, portletDataContext, assetVocabularyPKs, assetCategoryPKs, assetCategoryUuids);
    ...
    /**
     * Importe les categories à partir de childCategoryMap pour une catégorie parente spécifique.
     * 
     * @param childCategoryMap map des ids des catégories parents et des categories enfants
     * @param parentCategoryId id de la catégorie parente pour laquelle l'import doit être réalisé
     * @param categoryElements map des ids des catégories et des elements xml correspondant (comprenant les properties des catégories)
     * @param portletDataContext contexte de l'import
     * @param assetVocabularyPKs
     * @param assetCategoryPKs
     * @param assetCategoryUuids
     * @throws Exception
     */
    private void addAssetCategoriesRecursive(Map<Long, List<AssetCategory>> childCategoryMap, long parentCategoryId, Map<Long, Element> categoryElements, PortletDataContext portletDataContext, Map<Long, Long> assetVocabularyPKs, Map<Long, Long> assetCategoryPKs, Map<String, String> assetCategoryUuids) throws Exception {

        List<AssetCategory> childCategories = childCategoryMap.get(parentCategoryId);
        if (childCategories != null) {
            for (AssetCategory category : childCategories) {
              importAssetCategory(portletDataContext, assetVocabularyPKs, assetCategoryPKs, assetCategoryUuids, categoryElements.get(category.getCategoryId()), category);
              addAssetCategoriesRecursive(childCategoryMap, category.getCategoryId(), categoryElements, portletDataContext, assetVocabularyPKs, assetCategoryPKs, assetCategoryUuids);
            }
        }
    }
    // CUSTOM LMDE -- fin
