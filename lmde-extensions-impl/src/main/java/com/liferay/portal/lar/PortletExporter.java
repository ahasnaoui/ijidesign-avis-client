...
public class PortletExporter {
	...
	protected File doExportPortletInfoAsFile(
		...
		// CUSTOM LMDE -- debut
		String[] exportAllCategoriesParams = portletDataContext.getParameterMap().get("exportAllCategories");
		if (exportAllCategoriesParams != null && GetterUtil.getBoolean(exportAllCategoriesParams[0])) {
		    exportAllCategories(portletDataContext);
		}
		// CUSTOM LMDE -- fin
	...
	// CUSTOM LMDE -- debut
	protected void exportAllCategories(PortletDataContext portletDataContext) throws PortalException, SystemException, IOException {
	    
	    long scopeGroupId = portletDataContext.getScopeGroupId();
	    List<AssetVocabulary> assetVocabularies =
                AssetVocabularyLocalServiceUtil.getGroupsVocabularies(new long[]{scopeGroupId});
	    for (AssetVocabulary assetVocabulary : assetVocabularies) {
	        ...
	        for (AssetCategory assetCategory : assetCategories) {
	            ...
	            // Category
	            path = getAssetCategoryPath(portletDataContext, assetCategory.getCategoryId());
	            portletDataContext.addZipEntry(path, assetCategory);
	            ...
	            if (!assetCategoryProperties.isEmpty()) {
	                ...
	                Element propertiesElement = document.addElement("properties");

	                for (AssetCategoryProperty assetCategoryProperty :
	                        assetCategoryProperties) {
	                    Element propertyElement = propertiesElement.addElement(
	                        "property");
	                    propertyElement.addAttribute(
	                        "userUuid", assetCategoryProperty.getUserUuid());
	                    propertyElement.addAttribute("key", assetCategoryProperty.getKey());
	                ...
	                StringBundler propertiesPath = new StringBundler(5);
	                propertiesPath.append(ExportImportPathUtil.getRootPath(portletDataContext));
	                propertiesPath.append("/categories/properties/");
	                propertiesPath.append(assetCategory.getCategoryId());
	                propertiesPath.append("-properties");
	                propertiesPath.append(".xml");         
	                portletDataContext.addZipEntry(propertiesPath.toString(), document.formattedString());
	            }
	        }
	    }
	}
