...
public class LoginUtil {
    ...
    public static String getLogin(HttpServletRequest request, String paramName, Company company) throws SystemException {
        ...
        if ((login == null) || login.equals(StringPool.NULL)) {
            // DEBUT CUSTOM : Récupération depuis le cookie Identifiant
            login = GetterUtil.getString(CookieKeys.getCookie(request, LMDEConstants.IDENTIFIANT_COOKIE, false));
            /*
    ...
    public static void login(HttpServletRequest request, HttpServletResponse response, String login, String password, boolean rememberMe, String authType)
            throws Exception {
        ...
        // DEBUT CUSTOM
        // On met le screenName de l'utilisateur en session pour le récupérer depuis les portlets métiers sans invoker
        // les services Liferay
        final String screenName = user.getScreenName() != null ? user.getScreenName().toUpperCase() : null;
        session.setAttribute(LMDEConstants.USER_SCREENNAME, screenName);
        session.setAttribute(LMDEConstants.USER_LOGIN, user.getLogin());
        // FIN

        // DEBUT CUSTOM LMDE-252/LMDE-264
        Boolean succesPremiereAuthent;
        if ((LMDEUserUtil.isFirstLogin(userId) && !LMDEUserUtil.isInterne(user.getCompanyId(), userId)) || password.equals("firstLogin")) {
            if (password.equals("firstLogin")) {
                session.setAttribute(LMDEConstants.SUCCES_PREMIERE_AUTHENT_FACEBOOK, true);
            }
            try {
                EspaceAdherentServiceUtil.updateEmail(screenName, user.getLogin());
                succesPremiereAuthent = true;
                // Invalidation des caches si l'email a été mis à jour avec succes
            ...
            } catch (LmdeServiceEspaceAdherentException e) {
                LmdeAuthentificationTraceLocalServiceUtil.traceAuthentification(login, CauseEchecAuthentification.LMDE_EMAIL_UPDATE_ERROR,
                        "Première authentification de l'utilisateur externe. Erreur lors de la mise à jour de son e-mail dans le SI Intériale");
                succesPremiereAuthent = false;
            }
            session.setAttribute(LMDEConstants.SUCCES_PREMIERE_AUTHENT, succesPremiereAuthent);
        }
        ...
        // DEBUT CUSTOM
        // On ajoute le type d'authent en session pour ne pas rediriger directement vers l'espace perso avant de fermer
        // le pop-up facebook
        ...
        // DEBUT CUSTOM : Nouveau Cookie pour stocker l'identifiant de l'utilisateur (choix de ne pas utiliser le
        // loginCookie qui stocke la même info car on aurait été obliger de surcharger la LogoutAction pour le garder)
        Cookie identifiantCookie = new Cookie(LMDEConstants.IDENTIFIANT_COOKIE, login);

        if (Validator.isNotNull(domain)) {
            identifiantCookie.setDomain(domain);
        }

        if (rememberMe) {
            identifiantCookie.setMaxAge(loginMaxAge);
        } else {
            identifiantCookie.setMaxAge(0);
        }
        identifiantCookie.setPath(StringPool.SLASH);
        ...
        // DEBUT Custom : Ajout de identifiantCookie
        CookieKeys.addCookie(request, response, identifiantCookie, secure);
        // FIN CUSTOM
        ...
        session.setAttribute("facebookTokenOk", "f");
    ...
    public static void sendPassword(ActionRequest actionRequest) throws Exception {
        ...
        // CUSTOM begin
        String toAddressLMDE = (String) actionRequest.getAttribute("LMDE_toAddress");
        if (Validator.isNotNull(toAddressLMDE)) {
            toAddress = toAddressLMDE;
        }
        // CUSTOM end
    ...
    public static void sendPassword(ActionRequest actionRequest, String fromName, String fromAddress, String toAddress, String subject, String body)
            throws Exception {
        ...
        // CUSTOM begin
        serviceContext.setAttribute("LMDE_user", (User) actionRequest.getAttribute("LMDE_user"));
        // CUSTOM end
