 ...
 * Upon import, new instances of the corresponding articles, structures, and templates are created or updated according to the DATA_MIRROW strategy The author
 * of the newly created objects are determined by the JournalCreationStrategy class defined in <i>portal.properties</i>. That strategy also allows the text of
 * the journal article to be modified prior to import.
 * </p>
 * <p>
 * This <code>PortletDataHandler</code> differs from <code>JournalContentPortletDataHandlerImpl</code> in that it exports all articles owned by the group
 * whether or not they are actually displayed in a portlet in the layout set.
 * </p>
 *
 * @author Raymond Augé
...
public class JournalPortletDataHandler extends BasePortletDataHandler {
    ...
    public JournalPortletDataHandler() {
        ...
        setDeletionSystemEventStagedModelTypes(new StagedModelType(DDMStructure.class, JournalArticle.class), new StagedModelType(DDMTemplate.class,
                DDMStructure.class), new StagedModelType(JournalArticle.class), new StagedModelType(JournalArticle.class, DDMStructure.class),
                new StagedModelType(JournalFeed.class), new StagedModelType(JournalFolder.class));
        setExportControls(
                new PortletDataHandlerBoolean(NAMESPACE, "web-content", true, false, new PortletDataHandlerControl[] {
                        new PortletDataHandlerBoolean(NAMESPACE, "referenced-content"),
                        new PortletDataHandlerBoolean(NAMESPACE, "version-history", PropsValues.JOURNAL_PUBLISH_VERSION_HISTORY_BY_DEFAULT) },
                        JournalArticle.class.getName()), new PortletDataHandlerBoolean(NAMESPACE, "structures", true, false, null,
                        DDMStructure.class.getName(), JournalArticle.class.getName()), new PortletDataHandlerBoolean(NAMESPACE, "feeds", true, false, null,
    ...
    protected PortletPreferences doDeleteData(PortletDataContext portletDataContext, String portletId, PortletPreferences portletPreferences) throws Exception {
        ...
        JournalFolderLocalServiceUtil.deleteFolders(portletDataContext.getGroupId());
    ...
    protected String doExportData(final PortletDataContext portletDataContext, String portletId, PortletPreferences portletPreferences) throws Exception {
        ...
        if (portletDataContext.getBooleanParameter(NAMESPACE, "structures")) {
            ...
            ActionableDynamicQuery ddmStructureActionableDynamicQuery = getDDMStructureActionableDynamicQuery(portletDataContext, ddmTemplates, true);
            ...
            ActionableDynamicQuery ddmTemplateActionableDynamicQuery = getDDMTemplateActionableDynamicQuery(portletDataContext);

            ddmTemplateActionableDynamicQuery.performActions();

            // Export DDM structure default values

            ActionableDynamicQuery ddmStructureDefaultValuesActionableDynamicQuery = getDDMStructureDefaultValuesActionableDynamicQuery(portletDataContext);
        ...
        if (portletDataContext.getBooleanParameter(NAMESPACE, "web-content")) {
            ActionableDynamicQuery folderActionableDynamicQuery = new JournalFolderExportActionableDynamicQuery(portletDataContext);
    ...
    protected void doPrepareManifestSummary(PortletDataContext portletDataContext, PortletPreferences portletPreferences) throws Exception {
        ...
        ActionableDynamicQuery ddmStructureActionableDynamicQuery = getDDMStructureActionableDynamicQuery(portletDataContext, ddmTemplates, false);
        ...
        manifestSummary.addModelAdditionCount(new StagedModelType(DDMTemplate.class, DDMStructure.class), ddmTemplateActionableDynamicQuery.performCount()
                + ddmTemplates.size());
    ...
    protected ActionableDynamicQuery getArticleActionableDynamicQuery(final PortletDataContext portletDataContext) throws SystemException {
        ...
        return new JournalArticleExportActionableDynamicQuery(portletDataContext) {
            ...
            public void addCriteria(DynamicQuery dynamicQuery) {
                ...
                DynamicQuery articleVersionDynamicQuery = DynamicQueryFactoryUtil.forClass(JournalArticle.class, "articleVersion",
                        PortalClassLoaderUtil.getClassLoader());

                articleVersionDynamicQuery.setProjection(ProjectionFactoryUtil.alias(ProjectionFactoryUtil.max("articleVersion.version"),
                ...
                /*
                 * Debut custom LMDE 522: exporter uniquement les web content ayant une structure OFFRE
                 * Et LMDE 687: Diagnostic Sécurité Sociale - Gestion des contenus - Export/Import
                 */
                List<String> strutureIds = new ArrayList<>();

                if (portletDataContext.getBooleanParameter(NAMESPACE, "structure-offre-only", false)) {
                    strutureIds.add("LMDE_OFFRE");
                }
                if (portletDataContext.getBooleanParameter(NAMESPACE, "structure-aide-regionale-only", false)) {
                    strutureIds.add("LMDE_AIDE_REGIONALE");
                }
                if (portletDataContext.getBooleanParameter(NAMESPACE, "structure-diag-secu-resultat-only", false)) {
                    strutureIds.add("LMDE_DIAG_SECU_RESULTATS");
                }
                if (portletDataContext.getBooleanParameter(NAMESPACE, "structure-diag-secu-recommandation-only", false)) {
                    strutureIds.add("LMDE_DIAG_SECU_RECOMMANDATION");
                }

                if (strutureIds.size() > 0) {
                    articleVersionDynamicQuery.add(RestrictionsFactoryUtil.in("this.structureId", strutureIds));
                }
                // Fin custom
    ...
    protected ActionableDynamicQuery getDDMStructureActionableDynamicQuery(final PortletDataContext portletDataContext, final List<DDMTemplate> ddmTemplates,
            final boolean export) throws SystemException {
        ...
        return new DDMStructureExportActionableDynamicQuery(portletDataContext) {
            ...
            protected void performAction(Object object) throws PortalException {
                ...
                try {
                    List<DDMTemplate> ddmStructureDDMTemplates = DDMTemplateLocalServiceUtil.getTemplatesByClassPK(ddmStructure.getGroupId(),
    ...
    protected ActionableDynamicQuery getDDMStructureDefaultValuesActionableDynamicQuery(final PortletDataContext portletDataContext) throws SystemException {
    ...
    protected ActionableDynamicQuery getDDMTemplateActionableDynamicQuery(final PortletDataContext portletDataContext) throws SystemException {
